...
const ServerHandler = require('../classes/utils/ServerHandler');
...
const callbackAction = async (interaction, client, distube) => {
	...
	try {
		...
		if (
			fileProcess === undefined ||
			filePath === undefined ||
			fileExe === undefined
		) {
			return interaction.editReply(
				'The server is not configured correctly, please contact an admin to check the config file.'
			);
		}
		const serverHandler = new ServerHandler(fileProcess, filePath, fileExe);
		...
		if (await serverHandler.checkProcess(fileProcess)) {
			return interaction.editReply(`${fileProcess} has already started.`);
		}
		/**
